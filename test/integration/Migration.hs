...
share
  [mkPersist sqlSettings, mkMigrate "autoMigration"]
  [persistLowerCase|
Person
  name String
  hometown CityId
...
manualMigration =
  ...
  [ 0 ~> 1
    ...
    3 ~> 4
      := [ AddColumn
             "person"
             (Column "colorblind" SqlBool [NotNull])
             (Just $ PersistBool False)
         ]
  ]
  where
    migrateGender =
      RawOperation "Convert binary sex column into stringly gender column" $
...
testMigrations dir backend getPool =
  testGroup
    "goldens"
    [ testMigration' "Migrate from empty" 0 [],
      testMigration'
        "Migrate with v1 person"
        1
        [ insertPerson "David" []
        ],
      testMigration'
        "Migrate from sex to gender"
        2
      ...
      testMigration'
        "Migrate with default colorblind"
        4
        [ insertPerson "David" []
        ],
      testMigration'
        "Migrations are idempotent"
        (length manualMigration)
